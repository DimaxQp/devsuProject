...
public class Base_PO {
    ...
    /**
     * Nuevas funciones genéricas agregadas
     * Autor: Jorge Montero
     * Acceptar alerta.
     */
...
//	Falta definir las rutas de entrada para los archivos Excel para importar (dependiendo del país)

/*    public String getArchivo(String nombreArchivoEntrada) {
        String nombreArchivoSalida = "";
        String pais = DriverFactory.dotenv.get("PAIS");

        switch (pais) {
            case "Chile":
                nombreArchivoSalida = "src/test/resources/data/Chile" + nombreArchivoEntrada;
                break;
            case "Colombia":
                nombreArchivoSalida = "src/test/resources/data/Colombia" + nombreArchivoEntrada;
                break;
            case "Mexico":
                nombreArchivoSalida = "src/test/resources/data/Mexico" + nombreArchivoEntrada;
                break;
            case "Peru":
                nombreArchivoSalida = "src/test/resources/data/Peru" + nombreArchivoEntrada;
                break;
            case "Performance_M":
                nombreArchivoSalida = "src/test/resources/data/Performance_M" + nombreArchivoEntrada;
                break;
        }
        return nombreArchivoSalida;
    }

    public void uploadfile(String nombreArchivoEntrada) {
        String nombreArchivoSalida = getArchivo(nombreArchivoEntrada);
        File rutaArchivo = new File(nombreArchivoSalida);

        WebElement uploadFile = getDriver().findElement(By.xpath("//input[@type='file']"));

        uploadFile.sendKeys(rutaArchivo.getAbsolutePath());
    ...
    public void EsperarElementosMinimos(By elemento, int cantidad) {
        WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(Constants_Vars.DEFAULT_TIMEOUT));
        wait.until(ExpectedConditions.numberOfElementsToBeMoreThan(elemento, cantidad -1));
    }
